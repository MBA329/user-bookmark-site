...
export const updateBookmark = async (req:Request,res:Response)=>{
  ...
  try{
    ...
    if (error.message && error.message.startsWith("Validation error")){return res.status(400).json({error:error.message})}
...
export const createBookmark = async(req:Request,res:Response)=>{
  ...
  try {
    ...
    if (!req.body.url || !req.body.title || !req.body.tags){
      return res.status(400).json({error:"missing required fields: url, title,collectionId"})
